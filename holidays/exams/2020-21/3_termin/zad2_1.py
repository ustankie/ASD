...
def double_prefix( L ):
    ...
    def rec(p,word):
        ...
        if (p.right==None or p.left==None) and not p.end:
            p.is_prefix=False
            return p.is_prefix

        if p.right!=None and p.left!=None:
            p.is_prefix=not (rec(p.left,word) or rec(p.right,word))
            if p.is_prefix:
                R.append(word)

        elif (p.right!=None or p.left!=None) and p.end:
            p.is_prefix=not rec(p.left,word) and not rec(p.right,word)
            if p.is_prefix:
                R.append(word)
        else:
            p.is_prefix=False
