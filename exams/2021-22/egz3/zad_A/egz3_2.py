...
def printTree(root, is_left=True, has_right=True, has_left=True, prefix="    ",):
    ...
    if is_left:
        if root.left != None:
            ...
            print(prefix)
        print(prefix + '└──>', root.key, " : ", root.span, " : ", root.data, " : ",root.cnt)
    else:
        print(prefix + '┌──>', root.key, " : ", root.span, " : ", root.data, " : ",root.cnt)
        if root.right != None:
            print(prefix+"    |   ")
...
def insert_range(root, rang, tmp):
    if root.span == tmp or (root.key=='leave' and (tmp[0]-1==root.span[0] or tmp[1]-1==root.span[1])) :
...
def dump_ranges(root, x):
    ...
    while not A.empty():
        ...
        if a != B[len(B)-1]:
            B.append(a)
    return B
def count_height(root,a):
    root.cnt=len(root.data)+a
    if root.right!=None:
        count_height(root.right,root.cnt)
    if root.left!=None:
        count_height(root.left,root.cnt)
